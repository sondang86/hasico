...
$doc->addScript(JURI::root().'templates/'.$template.'/tclibs/helper/jquery-1.11.1.min.js');
}else{
$doc->addStyleSheet(JURI::root().'media/jui/css/chosen.css');
$doc->addScript(JURI::root().'media/jui/js/chosen.jquery.min.js');
$doc->addScriptDeclaration( 'jQuery.noConflict(); jQuery(document).ready(function() {jQuery("#jform_params_menutype, .mx-chosen-select").chosen();});' );
}
...
jQuery('.font1,.font2', '#style-form').closest('li, .control-group').hide(); 
jQuery('#jform_params_layoutscript', '#style-form').parent().addClass('mx-alert');
...
jQuery('.fontType > input', '#style-form').click(function(){jQuery('.font1,.font2', '#style-form').closest('li, .control-group').hide(); 		jQuery('.font'+jQuery(this).val()).closest('li, .control-group').show();});";
$html[] = '});</script></div>';
return implode($html);
}
}
