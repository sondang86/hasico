	...
	$.fn.extend({
		eFuncDr: function(options) {
			...
			return this.each(function(){
			    ...
			    obj.append('<div class="mx-blocks clearfix" />').addClass('mx-blocks-container clearfix');
				for (var i=0; i<opts.gridCols; i++){
					var posItem = '<span class="colnumber">'+colsNumber+'</span><span class="colname">'+blockName+(i+1)+'</span>';
					obj.children('.mx-blocks').append('<div class="block-grid tcGrid'+colsNumber+'"><div class="mx-block-content"><span class="mx-edit-block">Edit</span><span class="mx-devides"><i class="d">1</i><i class="t">1</i><i class="m">1</i><b class="customClass">&nbsp;</b></span><div class="mx-content-inside">'+posItem+'</div></div></div>');
				}
				var blocks = $('.mx-blocks > div', obj),
				...
				for (var i=0; i <= current; i++){
					...
					blocks.eq(i).find('.mx-devides > i').each(function(index, element) {
				...
				//obj.children('.mx-blocks').sortable();
				obj.find('.mx-edit-block').click(function(e) {
					var tcparent = $(this).parent();
					$(this).addClass('mx-edit-block-in');
                    tcparent.append('<div class="mx-popover right"><div class="arrow"></div><h3 class="popover-title">'+opts.popupTitle+'</h3><div class="popover-content">'+opts.popupContent+'</div></div>');
					$('.mx-popover',obj).find('select option').filter(function() { 
						return $(this).val() == tcparent.find('.colname').text(); 
					}).attr('selected', true);
					tcparent.find('.mx-devides > i').each(function(index, element) {
                        if(parseInt($(this).text()) == 1)
							$('.mx-popover',obj).find('.visibleDevaices > input').eq(index).attr("checked", "checked");
                    });
					$('.mx-popover',obj).find('.inputClass').attr('value', tcparent.find('.mx-devides > b').text());
					if($(this).nextAll('.mx-popover').is(':hidden'))
						tcparent.children('.mx-popover').show();
                });
				$(document).mouseup(function(e) {
					var currentValue = textarea.text().split("|");
					var currentPosValue = currentValue[1].split(",");
					if(!$(e.target, obj).parents('.mx-popover').length || $(e.target, obj).is('button')){
					    obj.mouseup(function(e){
							if($(e.target, obj).is('button') && $(e.target, obj).hasClass('save')){
								var posText = $(e.target, obj).parents('.mx-popover').find('select option:checked').attr('value');
								var customClassText = $(e.target, obj).parents('.mx-popover').find('.inputClass').val();
								...
								$('.mx-popover',obj).find('.visibleDevaices > input').each(function(index, element) {
									if($(this).is(':checked'))
										targetBlock.find('.mx-devides > i').eq(index).text('1').addClass('disable');
									else
										targetBlock.find('.mx-devides > i').eq(index).text('0').removeClass('disable');
								});
						...
						obj.find('.mx-popover').remove().end().find('.mx-edit-block').removeClass('mx-edit-block-in');
					}
                });
				
				obj.find('.block-grid').resizable({
					containment: obj.children('.mx-blocks'),
					grid: opts.gridStep,
					handles: 'e',
					stop: function() {
						var currentValue = textarea.text().split("|");
