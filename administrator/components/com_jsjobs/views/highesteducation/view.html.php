...
//        layout start
        if ($layoutName == 'formhighesteducation') {          // highest educations
            ...
            JToolBarHelper :: deleteList(JText::_('JS_ARE_YOU_SURE'),'highesteducation.remove');
            $result = $this->getJSModel('highesteducation')->getAllHighestEducations($limitstart, $limit);
            $items = $result[0];
            $total = $result[1];
            if ($total <= $limitstart)
                $limitstart = 0;
